...
const alertVariants = cva(
  "tailwind.config.tsrelative tailwind.config.tsw-full tailwind.config.tsrounded-lg tailwind.config.tsborder tailwind.config.tsp-4 [&>svg~*]:tailwind.config.tspl-7 [&>svg+div]:tailwind.config.tstranslate-y-[-3px] [&>svg]:tailwind.config.tsabsolute [&>svg]:tailwind.config.tsleft-4 [&>svg]:tailwind.config.tstop-4 [&>svg]:tailwind.config.tstext-foreground",
  {
    variants: {
      variant: {
        default: "tailwind.config.tsbg-background tailwind.config.tstext-foreground",
        destructive:
...
>(({ className, ...props }, ref) => (
  <h5
    ref={ref}
    className={cn("tailwind.config.tsmb-1 tailwind.config.tsfont-medium tailwind.config.tsleading-none tailwind.config.tstracking-tight", className)}
    {...props}
  />
))
...
>(({ className, ...props }, ref) => (
  <div
    ref={ref}
    className={cn("tailwind.config.tstext-sm [&_p]:tailwind.config.tsleading-relaxed", className)}
    {...props}
  />
))
